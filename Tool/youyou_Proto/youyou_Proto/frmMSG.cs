...
namespace ReadExcel
{
    public partial class frmMSG : Form
    {
        ...
        private void CreateServerProto(MyProto proto, bool isSave = false)
        {
                ...
                if (item.AttType != "byte" && item.AttType != "short" && item.AttType != "int" && item.AttType != "long" && item.AttType != "string" && item.AttType != "char" && item.AttType != "float" && item.AttType != "decimal" && item.AttType != "bool" && item.AttType != "ushort")
                {
                    ...
                    if (item.AttIsExistsProto)
                    {
                        ...
                        if (item.AttIsLoop)
                        {
                            ...
                            sbr.AppendFormat("    /// {0}\r\n", item.AttCnName);
                            sbr.AppendFormat("    /// </summary>\r\n");
                            sbr.AppendFormat("    public List<{0}Proto> {1};\r\n\r\n", item.AttType, item.AttEnName);
                        }
                        else
                        {
                            sbr.AppendFormat("    /// <summary>\r\n");
                            sbr.AppendFormat("    /// {0}\r\n", item.AttCnName);
                            sbr.AppendFormat("    /// </summary>\r\n");
                            sbr.AppendFormat("    public {0}Proto {1};\r\n\r\n", item.AttType, item.AttEnName);
                        }
                        #endregion
                    }
                    else
            ...
            #region ToArray 方法
            sbr.AppendFormat("    public byte[] ToArray(MMO_MemoryStream ms, bool isChild = false)\r\n");
            sbr.AppendFormat("    {{\r\n");
            sbr.AppendFormat("        ms.SetLength(0);\r\n");
            sbr.AppendFormat("        if (!isChild)\r\n");
            sbr.AppendFormat("        {{\r\n");
            ...
            foreach (var item in lst)
            {
                ...
                else if ((item.AttType == "byte" || item.AttType == "short" || item.AttType == "int" || item.AttType == "long") && item.AttIsLoop)
                {
                    ...
                    //查找隶属于这个循环项的
                    {
                        ...
                        if (list != null && list.Count > 0)
                        {
                            sbr.AppendFormat("            var item = {0}List[i];\r\n", list[0].AttEnName);
                ...
                else if (item.AttIsExistsProto)
                {
                    ...
                    if (item.AttIsLoop)
                    {
                        ...
                        sbr.AppendFormat("        {{\r\n");
                        sbr.AppendFormat("            int len_{0} = {0}.Count;\r\n", item.AttEnName);
                        sbr.AppendFormat("            ms.WriteInt(len_{0});\r\n", item.AttEnName);
                        sbr.AppendFormat("            for (int i = 0; i < len_{0}; i++)\r\n", item.AttEnName);
                        sbr.AppendFormat("            {{\r\n");
                        sbr.AppendFormat("                {0}Proto _{0}Proto = {1}[i];\r\n", item.AttType, item.AttEnName);
                        sbr.AppendFormat("                byte[] _buff_Curr = _{0}Proto.ToArray(new MMO_MemoryStream(), true);\r\n", item.AttType);
                        sbr.AppendFormat("                ms.WriteInt(_buff_Curr.Length);\r\n");
                        sbr.AppendFormat("                ms.Write(_buff_Curr, 0, _buff_Curr.Length);\r\n");
                        sbr.AppendFormat("            }}\r\n");
                        sbr.AppendFormat("        }}\r\n");
                        sbr.AppendFormat("        else\r\n");
                        sbr.AppendFormat("        {{\r\n");
                        sbr.AppendFormat("            ms.WriteInt(0);\r\n");
                        sbr.AppendFormat("        }}\r\n");
                    }
                    else
                    {
                        //不是循环项
                        sbr.AppendFormat("\r\n");
                        sbr.AppendFormat("        byte[] buff_{0} = {0}.ToArray(new MMO_MemoryStream(), true);\r\n", item.AttEnName);
                        sbr.AppendFormat("        ms.WriteInt(buff_{0}.Length);\r\n", item.AttEnName);
                        sbr.AppendFormat("        ms.Write(buff_{0}, 0, buff_{0}.Length);\r\n", item.AttEnName);
                    }
                }
            ...
            foreach (var item in lst)
            {
                ...
                else if (item.AttIsExistsProto)
                {
                    ...
                    if (item.AttIsLoop)
                    {
                        //如果是循环项
                        sbr.AppendFormat("\r\n");
                        sbr.AppendFormat("        int len_{0} = ms.ReadInt();\r\n", item.AttEnName);
                        sbr.AppendFormat("        if (len_{0} > 0)\r\n", item.AttEnName);
                        sbr.AppendFormat("        {{\r\n");
                        sbr.AppendFormat("            proto.{0} = new List<{1}Proto>();\r\n", item.AttEnName, item.AttType);
                        sbr.AppendFormat("            for (int i = 0; i < len_{0}; i++)\r\n", item.AttEnName);
                        sbr.AppendFormat("            {{\r\n");
                        sbr.AppendFormat("                int _len_{0} = ms.ReadInt();\r\n", item.AttEnName);
                        sbr.AppendFormat("                if (_len_{0} > 0)\r\n", item.AttEnName);
                        sbr.AppendFormat("                {{\r\n");
                        sbr.AppendFormat("                    byte[] _buff_{0} = new byte[_len_{0}];\r\n", item.AttEnName);
                        sbr.AppendFormat("                    ms.Read(_buff_{0}, 0, _len_{0});\r\n", item.AttEnName);
                        sbr.AppendFormat("                    proto.{0}.Add({1}Proto.GetProto(new MMO_MemoryStream(), _buff_{0}));\r\n", item.AttEnName, item.AttType);
                        sbr.AppendFormat("                }}\r\n");
                        sbr.AppendFormat("            }}\r\n");
                        sbr.AppendFormat("        }}\r\n");
                    }
                    else
                    {
                        ...
                        sbr.AppendFormat("        if (len_{0} > 0)\r\n", item.AttEnName);
                        sbr.AppendFormat("        {{\r\n");
                        sbr.AppendFormat("            byte[] buff_{0} = new byte[len_{0}];\r\n", item.AttEnName);
                        sbr.AppendFormat("            ms.Read(buff_{0}, 0, len_{0});\r\n", item.AttEnName);
                        sbr.AppendFormat("            proto.{0} = {1}Proto.GetProto(new MMO_MemoryStream(), buff_{0});\r\n", item.AttEnName, item.AttType);
                        sbr.AppendFormat("        }}\r\n");
                    }
                }
            }
        ...
        private void CreateProto(MyProto proto, bool isSave = false)
        {
            ...
            sbr.AppendFormat("using System.Collections.Generic;\r\n");
            sbr.AppendFormat("using System;\r\n");
            sbr.AppendFormat("using TQ;\r\n");
            sbr.Append("\r\n");
            sbr.Append("[Serializable]\r\n");
            sbr.AppendFormat("/// <summary>\r\n");
            sbr.AppendFormat("/// {0}\r\n", proto.ProtoCnName);
            sbr.AppendFormat("/// </summary>\r\n");
            sbr.AppendFormat("public struct {0}Proto : IProto\r\n", proto.ProtoEnName);
            ...
            #region ToArray 方法
            sbr.AppendFormat("    public byte[] ToArray(bool isChild = false)\r\n");
            sbr.AppendFormat("    {{\r\n");
            sbr.AppendFormat("        MMO_MemoryStream ms = null;\r\n");
            sbr.AppendFormat("        \r\n");
            sbr.AppendFormat("        if (!isChild)\r\n");
            sbr.AppendFormat("        {{\r\n");
            sbr.AppendFormat("            ms = GameEntry.Socket.SocketSendMS;\r\n");
            sbr.AppendFormat("            ms.SetLength(0);\r\n");
            sbr.AppendFormat("            ms.WriteUShort(ProtoCode);\r\n");
            sbr.AppendFormat("        }}\r\n");
            sbr.AppendFormat("        else\r\n");
            sbr.AppendFormat("        {{\r\n");
            sbr.AppendFormat("            ms = GameEntry.Pool.DequeueClassObject<MMO_MemoryStream>();\r\n");
            sbr.AppendFormat("            ms.SetLength(0);\r\n");
            sbr.AppendFormat("        }}\r\n");
            sbr.Append("\r\n");
            ...
            sbrLua.Append("        ms = CS.TQ.GameEntry.Socket.SocketSendMS;\r\n");
            sbrLua.Append("        ms:SetLength(0);\r\n");
            sbrLua.Append("        ms:WriteUShort(proto.ProtoCode);\r\n");
            sbrLua.Append("    else\r\n");
            sbrLua.Append("        ms = CS.TQ.GameEntry.Lua:DequeueMemoryStream();\r\n");
            sbrLua.Append("        ms.SetLength(0);\r\n");
            sbrLua.Append("    end\r\n\r\n");
            ...
            foreach (var item in lst)
            {
                ...
                else if (item.AttIsExistsProto)
                {
                    ...
                    if (item.AttIsLoop)
                    {
                        ...
                        sbr.AppendFormat("            {{\r\n");
                        sbr.AppendFormat("                {0}Proto _{0}Proto = {1}[i];\r\n", item.AttType, item.AttEnName);
                        sbr.AppendFormat("                byte[] _buff_Curr = _{0}Proto.ToArray(true);\r\n", item.AttType);
                        sbr.AppendFormat("                ms.WriteInt(_buff_Curr.Length);\r\n");
                        sbr.AppendFormat("                ms.Write(_buff_Curr, 0, _buff_Curr.Length);\r\n");
                        sbr.AppendFormat("            }}\r\n");
                        sbr.AppendFormat("        }}\r\n");
                    ...
                    else
                    {
                        //不是循环项
                        sbr.AppendFormat("\r\n");
                        sbr.AppendFormat("        byte[] buff_{0} = {0}.ToArray(true);\r\n", item.AttEnName);
                        sbr.AppendFormat("        ms.WriteInt(buff_{0}.Length);\r\n", item.AttEnName);
                        sbr.AppendFormat("        ms.Write(buff_{0}, 0, buff_{0}.Length);\r\n", item.AttEnName);
            ...
            sbr.Append("\r\n");
            sbr.AppendFormat("        byte[] retBuffer = ms.ToArray();\r\n");
            sbr.AppendFormat("        if (isChild)\r\n");
            sbr.AppendFormat("        {{\r\n");
            sbr.AppendFormat("            GameEntry.Pool.EnqueueClassObject(ms);\r\n");
            sbr.AppendFormat("        }}\r\n");
            sbr.AppendFormat("        return retBuffer;\r\n");
            sbr.AppendFormat("    }}\r\n");
            sbr.Append("\r\n");
            ...
            sbrLua.Append("\r\n");
            sbrLua.Append("    if (isChild == nil or isChild == false) then\r\n");
            sbrLua.Append("        if (CS.TQ.GameEntry.Lua.DebugLogProto == true) then\r\n");
            sbrLua.Append("            print(string.format(\"<color=#000000>发送消息:</color><color=#000000>%s %s</color>\", this.GetProtoName(), proto.ProtoCode));\r\n");
            sbrLua.Append("            print(string.format(\"<color=#000000>==>>%s</color>\", json.encode(proto)));\r\n");
            sbrLua.Append("        end\r\n");
            sbrLua.Append("\r\n");
            sbrLua.Append("        CS.TQ.GameEntry.Socket:SendMsg(ms:ToArray());\r\n");
            sbrLua.Append("    else\r\n");
            sbrLua.Append("        local retBuffer = ms:ToArray();\r\n");
            sbrLua.Append("        CS.TQ.GameEntry.Lua:EnqueueMemoryStream(ms);\r\n");
            ...
            #region GetProto 方法
            sbr.AppendFormat("    public static {0}Proto GetProto(byte[] buffer, bool isChild = false)\r\n", proto.ProtoEnName);
            sbr.AppendFormat("    {{\r\n");
            sbr.AppendFormat("        {0}Proto proto = new {0}Proto();\r\n", proto.ProtoEnName);
            sbr.Append("\r\n");
            sbr.AppendFormat("        MMO_MemoryStream ms = null;\r\n");
            ...
            sbrLua.AppendFormat("        ms = CS.TQ.GameEntry.Lua:LoadSocketReceiveMS(buffer);\r\n");
            sbrLua.AppendFormat("    else\r\n");
            sbrLua.AppendFormat("        ms = CS.TQ.GameEntry.Lua:DequeueMemoryStreamAndLoadBuffer(buffer);\r\n");
            sbrLua.AppendFormat("    end\r\n");
            sbrLua.Append("\r\n");
            //======================lua协议代码结束
            ...
            foreach (var item in lst)
            {
                ...
                else if (item.AttIsExistsProto)
                {
                    ...
                    if (item.AttIsLoop)
                    {
                        ...
                        sbr.AppendFormat("                    byte[] _buff_{0} = new byte[_len_{0}];\r\n", item.AttEnName);
                        sbr.AppendFormat("                    ms.Read(_buff_{0}, 0, _len_{0});\r\n", item.AttEnName);
                        sbr.AppendFormat("                    proto.{0}.Add({1}Proto.GetProto(_buff_{0}, true));\r\n", item.AttEnName, item.AttType);
                        sbr.AppendFormat("                }}\r\n");
                        sbr.AppendFormat("            }}\r\n");
                        sbr.AppendFormat("        }}\r\n");
                        ...
                        sbrLua.AppendFormat("                local _buff_{0} = CS.TQ.GameEntry.Lua:GetByteArray(ms, _len_{0});\r\n", item.AttEnName);
                    ...
                    else
                    {
                        ...
                        sbr.AppendFormat("            byte[] buff_{0} = new byte[len_{0}];\r\n", item.AttEnName);
                        sbr.AppendFormat("            ms.Read(buff_{0}, 0, len_{0});\r\n", item.AttEnName);
                        sbr.AppendFormat("            proto.{0} = {1}Proto.GetProto(buff_{0}, true);\r\n", item.AttEnName, item.AttType);
                        sbr.AppendFormat("        }}\r\n");
                        ...
                        sbrLua.AppendFormat("    local len_{0} = ms:ReadInt();\r\n", item.AttEnName);
                        sbrLua.AppendFormat("    if (len_{0} > 0) then\r\n", item.AttEnName);
                        sbrLua.AppendFormat("        local buff_{0} = CS.TQ.GameEntry.Lua:GetByteArray(ms, len_{0});\r\n", item.AttEnName);
            ...
            sbrLua.Append("\r\n");
            sbrLua.Append("    if (isChild == nil or isChild == false) then\r\n");
            sbrLua.AppendFormat("        if (CS.TQ.GameEntry.Lua.DebugLogProto == true) then\r\n");
            sbrLua.AppendFormat("            print(string.format(\"<color=#000000>接收消息:</color><color=#000000>%s %s</color>\", this.GetProtoName(), proto.ProtoCode));\r\n");
            sbrLua.AppendFormat("            print(string.format(\"<color=#000000>==>>%s</color>\", json.encode(proto)));\r\n");
            sbrLua.AppendFormat("        end\r\n");
            sbrLua.Append("    end\r\n");
            sbrLua.Append("\r\n");
        ...
        #region CreateProtoHandler 生成ProtoHandler
        /// <summary>
        /// 生成ProtoHandler
        /// </summary>
        /// <param name="proto"></param>
        ...
        private void CreateSocketProtoListener()
        {
            ...
            foreach (var menu in DataMgr.Instance.MenuList)
            {
                foreach (var proto in menu.ProtoList)
                {
                    ...
                    {
                        sbrLua.AppendFormat("    CS.TQ.GameEntry.Event.SocketEvent:AddEventListener(ProtoCode.{0}, {0}Handler.On{0});\r\n", proto.ProtoEnName);
                    }
                }
            }
        ...
        private void frmMSG_Load(object sender, EventArgs e)
        {
            ...
            lst.Add(new KeyValuePair<string, string>("C2MS/MS2C/C2SS", "全部"));
        ...
        private void txtProtoEnName_TextChanged(object sender, EventArgs e)
        {
